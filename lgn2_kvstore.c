 ...
 * lgn2-server owns the automatic byte-prefix cache policy built on top of this
 ...
 * lgn2_session internals become unsafe to restore across runtime changes. */
...
static const char *kv_log_name(const lgn2_kvstore *kc) {
    return kc && kc->log_name ? kc->log_name : "lgn2";
}

static void kv_logf(lgn2_kvstore *kc, lgn2_kvstore_log_type type,
                    const char *fmt, ...) {
    if (!kc || !kc->log) return;
    va_list ap;
    va_start(ap, fmt);
...
uint8_t lgn2_kvstore_reason_code(const char *reason) {
    if (!reason) return LGN2_KVSTORE_REASON_UNKNOWN;
    if (!strcmp(reason, "cold")) return LGN2_KVSTORE_REASON_COLD;
    if (!strcmp(reason, "continued")) return LGN2_KVSTORE_REASON_CONTINUED;
    if (!strcmp(reason, "evict")) return LGN2_KVSTORE_REASON_EVICT;
    if (!strcmp(reason, "shutdown")) return LGN2_KVSTORE_REASON_SHUTDOWN;
    if (!strcmp(reason, "agent-system")) return LGN2_KVSTORE_REASON_AGENT_SYSTEM;
    if (!strcmp(reason, "agent-session")) return LGN2_KVSTORE_REASON_AGENT_SESSION;
    return LGN2_KVSTORE_REASON_UNKNOWN;
}

const char *lgn2_kvstore_key_kind(uint8_t ext_flags) {
    if (ext_flags & LGN2_KVSTORE_EXT_RESPONSES_VISIBLE) return "responses-visible";
    if (ext_flags & LGN2_KVSTORE_EXT_THINKING_VISIBLE) return "thinking-visible";
    return "token-text";
}
...
char *lgn2_kvstore_path_for_sha(lgn2_kvstore *kc, const char sha[41]) {
    ...
    memcpy(name, sha, 40);
    memcpy(name + 40, ".kv", 4);
    return lgn2_kvstore_path_join(kc->dir, name);
}
...
void lgn2_kvstore_clear(lgn2_kvstore *kc) {
    for (int i = 0; i < kc->len; i++) lgn2_kvstore_entry_free(&kc->entry[i]);
    free(kc->entry);
    kc->entry = NULL;
    kc->len = 0;
    kc->cap = 0;
...
static void kv_cache_push(lgn2_kvstore *kc, lgn2_kvstore_entry e) {
    if (kc->len == kc->cap) {
...
void lgn2_kvstore_fill_header(uint8_t h[LGN2_KVSTORE_FIXED_HEADER],
    ...
    memset(h, 0, LGN2_KVSTORE_FIXED_HEADER);
    h[0] = KV_CACHE_MAGIC0;
    h[1] = KV_CACHE_MAGIC1;
    h[2] = KV_CACHE_MAGIC2;
    ...
    h[6] = ext_flags;
    h[7] = model_id;
    lgn2_kvstore_le_put32(h + 8, tokens);
    lgn2_kvstore_le_put32(h + 12, hits);
    lgn2_kvstore_le_put32(h + 16, ctx_size);
...
bool lgn2_kvstore_read_header(FILE *fp, lgn2_kvstore_entry *e,
                             uint32_t *text_bytes) {
    uint8_t h[LGN2_KVSTORE_FIXED_HEADER];
    ...
    e->quant_bits = h[4];
    e->reason = h[5] <= LGN2_KVSTORE_REASON_AGENT_SESSION ? h[5] :
                LGN2_KVSTORE_REASON_UNKNOWN;
    e->ext_flags = h[6];
    e->model_id = h[7];
    e->tokens = lgn2_kvstore_le_get32(h + 8);
    e->hits = lgn2_kvstore_le_get32(h + 12);
    e->ctx_size = lgn2_kvstore_le_get32(h + 16);
    ...
    *text_bytes = lgn2_kvstore_le_get32(tb);
...
bool lgn2_kvstore_read_entry_file(const char *path, const char sha[41],
    ...
    struct stat st;
    if (stat(path, &st) != 0 ||
        st.st_size < (off_t)(LGN2_KVSTORE_FIXED_HEADER + 4))
        return false;
    FILE *fp = fopen(path, "rb");
    if (!fp) return false;
    lgn2_kvstore_entry e = {0};
    uint32_t text_bytes = 0;
    bool ok = lgn2_kvstore_read_header(fp, &e, &text_bytes);
    fclose(fp);
    if (!ok) return false;
    const uint64_t fixed = LGN2_KVSTORE_FIXED_HEADER + 4ull;
...
static void kv_cache_refresh(lgn2_kvstore *kc) {
    if (!kc->enabled) return;
    lgn2_kvstore_clear(kc);
    DIR *d = opendir(kc->dir);
    if (!d) return;
    struct dirent *de;
    while ((de = readdir(d)) != NULL) {
        char sha[41];
        if (!lgn2_kvstore_sha_hex_name(de->d_name, sha)) continue;
        char *path = lgn2_kvstore_path_join(kc->dir, de->d_name);
        lgn2_kvstore_entry e = {0};
        if (lgn2_kvstore_read_entry_file(path, sha, &e)) kv_cache_push(kc, e);
        free(path);
    }
    closedir(d);
}
...
bool lgn2_kvstore_touch_file(const char *path, uint32_t hits) {
    ...
    if (ok) {
        ...
        lgn2_kvstore_fill_header(h, e.model_id, e.quant_bits, e.reason, e.ext_flags,
...
static bool kv_cache_incoming_supersedes_continued(
    ...
    char prefix_sha[41];
    lgn2_kvstore_sha1_bytes_hex(incoming->text, (size_t)e->text_bytes,
                               prefix_sha);
    return !strcmp(prefix_sha, e->sha);
}

static bool kv_cache_reason_is_anchor(uint8_t reason) {
    return reason == LGN2_KVSTORE_REASON_COLD ||
           reason == LGN2_KVSTORE_REASON_EVICT ||
           reason == LGN2_KVSTORE_REASON_SHUTDOWN;
}

double lgn2_kvstore_entry_eviction_score(
        const lgn2_kvstore_entry *e,
    ...
    if (used_at == 0) {
        ...
        effective_hits *= exp2(-elapsed / (double)LGN2_KVSTORE_HIT_HALF_LIFE_SECONDS);
...
void lgn2_kvstore_evict(lgn2_kvstore *kc, const lgn2_tokens *live,
    ...
    while (total > target && kc->len > 0) {
        int victim = 0;
        double victim_score =
            lgn2_kvstore_entry_eviction_score(&kc->entry[0], live, now,
                                             incoming);
        for (int i = 1; i < kc->len; i++) {
            double score =
                lgn2_kvstore_entry_eviction_score(&kc->entry[i], live, now,
                                                 incoming);
        ...
        lgn2_kvstore_entry e = kc->entry[victim];
        if (unlink(e.path) == 0) {
            kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
...
bool lgn2_kvstore_open(lgn2_kvstore *kc, const char *dir, uint64_t budget_mb,
    ...
    kc->log_ud = log_ud;
    if (!kv_mkdir_p(dir)) {
        kv_logf(kc, LGN2_KVSTORE_LOG_DEFAULT,
    ...
    kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
            ...
            (unsigned long long)LGN2_KVSTORE_HIT_HALF_LIFE_SECONDS);
    return true;
}
...
char *lgn2_kvstore_render_tokens_text(lgn2_engine *engine,
    ...
    for (int i = 0; i < tokens->len; i++) {
        size_t len = 0;
        char *piece = lgn2_token_text(engine, tokens->v[i], &len);
...
void lgn2_kvstore_tokens_copy_prefix(lgn2_tokens *dst, const lgn2_tokens *src, int n) {
    ...
    for (int i = 0; i < n; i++) lgn2_tokens_push(dst, src->v[i]);
}

static void tokens_append(lgn2_tokens *dst, const lgn2_tokens *src) {
    if (!dst || !src) return;
    for (int i = 0; i < src->len; i++) lgn2_tokens_push(dst, src->v[i]);
}
...
void lgn2_kvstore_build_prompt_from_exact_prefix_and_text_suffix(
        ...
        lgn2_tokens *out) {
    lgn2_tokens_copy(out, exact_prefix);

    lgn2_tokens suffix = {0};
    /* The suffix may start with LGN2 chat markers such as <｜User｜> or
     * </think>, so use the rendered-chat tokenizer, not plain text BPE. */
    lgn2_tokenize_rendered_chat(engine, suffix_text ? suffix_text : "", &suffix);
...
static int kv_cache_continued_step(const lgn2_kvstore *kc) {
...
int lgn2_kvstore_suppress_continued_store(lgn2_kvstore *kc, int tokens) {
    if (lgn2_kvstore_continued_store_target(kc, tokens) != tokens) return -1;
    int old = kc->continued_last_store_tokens;
    lgn2_kvstore_note_store(kc, tokens);
    return old;
}
...
static bool kv_cache_file_text_matches(const char *path, const char sha[41],
    ...
    lgn2_kvstore_entry hdr = {0};
    uint32_t text_bytes = 0;
    bool ok = lgn2_kvstore_read_header(fp, &hdr, &text_bytes) &&
    ...
    char stored_sha[41];
    lgn2_kvstore_sha1_bytes_hex(stored, text_bytes, stored_sha);
...
static bool kv_cache_existing_compatible(lgn2_kvstore *kc, const char *path,
                                         const char sha[41],
                                         const char *text, size_t text_len,
                                         int model_id, int quant_bits, int ctx_size) {
    if (access(path, F_OK) != 0) return false;
    lgn2_kvstore_entry e = {0};
    if (!lgn2_kvstore_read_entry_file(path, sha, &e)) return false;
...
static bool kv_trailer_serialized_size(const lgn2_kvstore_trailer_hooks *hooks,
                                       const char *text,
                                       uint64_t *bytes_out) {
...
static bool kv_trailer_write(const lgn2_kvstore_trailer_hooks *hooks,
...
static void kv_cache_rewrite_trailer(lgn2_kvstore *kc, const char *path,
                                     const char *text,
    ...
    uint32_t text_bytes = 0;
    bool ok = lgn2_kvstore_read_header(fp, &hdr, &text_bytes);
    uint64_t end = LGN2_KVSTORE_FIXED_HEADER + 4ull +
        ...
        ftruncate(fileno(fp), (off_t)end) == 0)
    {
        ...
        if (ok && ignored > 0) {
            uint8_t h[LGN2_KVSTORE_FIXED_HEADER];
            uint64_t now = (uint64_t)time(NULL);
            lgn2_kvstore_fill_header(h, hdr.model_id, hdr.quant_bits, hdr.reason,
...
bool lgn2_kvstore_store_live_prefix_text(lgn2_kvstore *kc,
    ...
    lgn2_tokens store_tokens = {0};
    lgn2_kvstore_tokens_copy_prefix(&store_tokens, tokens, store_len);

    const int quant_bits = lgn2_engine_routed_quant_bits(engine);
    if (quant_bits != 2 && quant_bits != 4) {
        lgn2_tokens_free(&store_tokens);
        return false;
    }
    const int model_id = lgn2_engine_model_id(engine);

    char save_err[160] = {0};
    const lgn2_tokens *live_tokens = lgn2_session_tokens(session);
    if (!live_tokens ||
        live_tokens->len != store_tokens.len ||
        !lgn2_tokens_starts_with(live_tokens, &store_tokens))
    {
        kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
    ...
    if (text_override) {
        ...
        text = lgn2_kvstore_render_tokens_text(engine, &store_tokens, &text_len);
    }
    ...
    char sha[41];
    lgn2_kvstore_sha1_bytes_hex(text, text_len, sha);
    char *path = lgn2_kvstore_path_for_sha(kc, sha);
    const uint8_t reason_code = lgn2_kvstore_reason_code(reason);

    if (kv_cache_existing_compatible(kc, path, sha, text, text_len,
                                     model_id,
                                     quant_bits, lgn2_session_ctx(session))) {
    ...
    lgn2_session_payload_file staged = {0};
    if (lgn2_session_stage_payload(session, &staged,
    ...
    if (!lgn2_kvstore_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                    trailer_est_bytes,
        ...
        lgn2_session_payload_file_free(&staged);
        free(text);
        free(path);
        lgn2_tokens_free(&store_tokens);
        return false;
    ...
    lgn2_kvstore_eviction_context incoming = {
        ...
        .ctx_size = (uint32_t)lgn2_session_ctx(session),
        .reject_different_quant = kc->reject_different_quant,
    };
    lgn2_kvstore_evict(kc, live_tokens, est_file_bytes, &incoming);
    ...
    lgn2_kvstore_fill_header(h, (uint8_t)model_id, (uint8_t)quant_bits,
                            reason_code, ext_flags,
                            (uint32_t)store_tokens.len, 0,
                            (uint32_t)lgn2_session_ctx(session),
                            now, now, payload_bytes);
    uint8_t tb[4];
    lgn2_kvstore_le_put32(tb, (uint32_t)text_len);
    uint64_t trailer_bytes = 0;
    errno = 0;
    bool ok = fwrite(h, 1, sizeof(h), fp) == sizeof(h) &&
              fwrite(tb, 1, sizeof(tb), fp) == sizeof(tb) &&
              fwrite(text, 1, text_len, fp) == text_len &&
              lgn2_session_write_staged_payload(&staged, fp,
                                               save_err, sizeof(save_err)) == 0 &&
    ...
    if (ok && !lgn2_kvstore_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                          trailer_bytes,
                                          &final_file_bytes,
    ...
    if (!ok) {
        ...
        kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
                ...
                (double)(LGN2_KVSTORE_FIXED_HEADER + 4ull + text_len + payload_bytes + trailer_bytes) / (1024.0 * 1024.0),
                save_ms);
    }
    lgn2_session_payload_file_free(&staged);
    free(tmp);
...
bool lgn2_kvstore_store_live_prefix(lgn2_kvstore *kc,
                                   ...
                                   char *err,
                                   size_t err_len) {
    return lgn2_kvstore_store_live_prefix_text(kc, engine, session, tokens,
...
bool lgn2_kvstore_maybe_store_continued(lgn2_kvstore *kc,
                                       ...
                                       char *err,
                                       size_t err_len) {
    const lgn2_tokens *tokens = lgn2_session_tokens(session);
    if (!tokens) return false;
    const int target = lgn2_kvstore_continued_store_target(kc, tokens->len);
    if (target == 0) return false;
    if (lgn2_kvstore_store_live_prefix(kc, engine, session, tokens, target,
                                      "continued", hooks, err, err_len))
    {
        lgn2_kvstore_note_store(kc, target);
        return true;
    }
    return false;
}
...
int lgn2_kvstore_find_text_prefix(lgn2_kvstore *kc, const char *prompt_text,
    ...
    for (int i = 0; i < kc->len; i++) {
        lgn2_kvstore_entry *e = &kc->entry[i];
        ...
        if (best >= 0) {
            lgn2_kvstore_entry *b = &kc->entry[best];
            if (e->text_bytes < b->text_bytes) continue;
            if (e->text_bytes == b->text_bytes && e->tokens <= b->tokens) continue;
        }
        char sha[41];
        lgn2_kvstore_sha1_bytes_hex(prompt_text, (size_t)e->text_bytes, sha);
...
int lgn2_kvstore_try_load_text(lgn2_kvstore *kc,
    ...
    const int model_id = lgn2_engine_model_id(engine);
    const size_t prompt_bytes = strlen(prompt_text);
    int idx = lgn2_kvstore_find_text_prefix(kc, prompt_text, model_id, quant_bits,
                                           lgn2_session_ctx(session));
    if (idx < 0) return 0;

    lgn2_kvstore_entry e = kc->entry[idx];
    ...
    bool header_ok = lgn2_kvstore_read_header(fp, &hdr, &text_bytes);
    ...
    if (header_ok) {
        if (hdr.model_id != (uint8_t)model_id) {
            ...
            if (fread(cached_text, 1, text_bytes, fp) != text_bytes) {
                ...
                char text_sha[41];
                lgn2_kvstore_sha1_bytes_hex(cached_text, text_bytes, text_sha);
                if (strcmp(text_sha, e.sha)) {
                    header_ok = false;
                    fail_reason = "cached text hash mismatch";
                } else if (!lgn2_kvstore_byte_prefix_match(prompt_text, prompt_bytes,
    ...
    int loaded = 0;
    if (header_ok &&
        lgn2_session_load_payload(session, fp, hdr.payload_bytes, err, sizeof(err)) == 0)
    {
        const lgn2_tokens *loaded_tokens = lgn2_session_tokens(session);
        if (loaded_tokens && loaded_tokens->len == (int)hdr.tokens) {
            ...
            lgn2_session_invalidate(session);
            unlink(path);
            kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
        ...
        if (header_ok) lgn2_session_invalidate(session);
        kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
    ...
    if (loaded > 0) {
        ...
        const char *key_kind = lgn2_kvstore_key_kind(hdr.ext_flags);
        lgn2_kvstore_touch_file(path, hdr.hits + 1);
        kv_logf(kc, LGN2_KVSTORE_LOG_KVCACHE,
