// LGN2 Metal get-rows kernel.

struct lgn2_metal_args_get_rows {
    int32_t  ne00t;
    int32_t  ne00;
    uint64_t nb01;
    uint64_t nb02;
...
struct lgn2_metal_args_get_rows_q8_0 {
    int32_t  n_embd;
    int32_t  n_vocab;
    int32_t  n_tokens;
...
struct lgn2_get_rows_block_q4_0 {
    half d;
    uchar qs[16];
};

struct lgn2_get_rows_block_q4_K {
    half d;
    half dmin;
    uchar scales[12];
    uchar qs[128];
...
static inline uchar2 lgn2_get_rows_q4_K_scale_min(int j, int k, device const uchar *q) {
    return j < 4 ? uchar2{uchar(q[j + 0 + k] & 63), uchar(q[j + 4 + k] & 63)}
                 : uchar2{uchar((q[j + 4 + k] & 0x0f) | ((q[j - 4 + k] & 0xc0) >> 2)),
                          uchar((q[j + 4 + k] >> 4) | ((q[j - 0 + k] & 0xc0) >> 2))};
}

// Gathers embedding/table rows by integer ids. LGN2 uses this for token
// embeddings and small indexed tables such as router/hash lookup outputs.
template<typename T0, typename T>
kernel void kernel_get_rows_f(
        constant lgn2_metal_args_get_rows & args,
...
kernel void kernel_get_rows_q8_0_f32(
        constant lgn2_metal_args_get_rows_q8_0 & args,
        device const char    * src0,
        device const char    * src1,
        device       char    * dst,
        uint3                  tgpig[[threadgroup_position_in_grid]],
...
kernel void kernel_get_rows_q4_0_f32(
    ...
    const device lgn2_get_rows_block_q4_0 *row =
        (const device lgn2_get_rows_block_q4_0 *)(src0 + (uint64_t)token * args.src_row_bytes);
    const device lgn2_get_rows_block_q4_0 *qb = row + block;
    device float *out =
        (device float *)(dst + (uint64_t)tok_i * args.dst_row_bytes);
...
kernel void kernel_get_rows_q4_K_f32(
    ...
    const device lgn2_get_rows_block_q4_K *row =
        (const device lgn2_get_rows_block_q4_K *)(src0 + (uint64_t)token * args.src_row_bytes);
    const device lgn2_get_rows_block_q4_K *qb = row + block;
    device float *out =
        (device float *)(dst + (uint64_t)tok_i * args.dst_row_bytes);
    ...
    for (int32_t i = (int32_t)tiitg; i < 256; i += (int32_t)ntg.x) {
        ...
        if (idx < args.n_embd) {
            ...
            const uchar2 sm = lgn2_get_rows_q4_K_scale_min((int)group, 0, qb->scales);
