...
static inline uint32_t lgn2_gpu_q8_mv_ext_max_tokens_parse(
        const char *value) {
...
static inline uint32_t lgn2_gpu_laguna_moe_min_tokens_parse(
        const char *value) {
...
static inline int lgn2_gpu_laguna_direct_kv_prefill_env_mode(
        const char *value) {
    if (!value || value[0] == '\0' ||
...
static inline int lgn2_gpu_laguna_dense_q8_gate_up_swiglu_env_mode(
        const char *value) {
    if (!value || value[0] == '\0' ||
        (value[0] == '0' && value[1] == '\0')) return 0;
    if (value[0] == '1' && value[1] == '\0') return 1;
 ...
 * than normalized. */
static inline int lgn2_gpu_laguna_dense_q8_gate_up_swiglu_rows_env_mode(
        const char *value) {
...
static inline int lgn2_gpu_laguna_dense_q8_gate_up_swiglu_preflight_decision(
        int selector_mode,
...
enum lgn2_gpu_laguna_dense_q8_gate_up_swiglu_route {
    LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_STOCK = 0,
    LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_DECODE_MID = 1,
    LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_ORDINARY_PREFILL_STOCK = 2,
    LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_BATCH_FUSED = 3,
};

/* Select the route after preflight.  Exact verifier rows map to generic stock;
 * only one-token decode uses the fused route. */
static inline int lgn2_gpu_laguna_dense_q8_gate_up_swiglu_route(
        int enabled, int decode, int exact_rows) {
    if (!enabled) return LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_STOCK;
    if (exact_rows) return LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_STOCK;
    if (decode) return LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_DECODE_MID;
    return LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_ORDINARY_PREFILL_STOCK;
}
...
static inline int lgn2_gpu_laguna_dense_q8_gate_up_swiglu_prefill_route(
        int enabled,
        int exact_rows,
        uint32_t n_tokens,
    ...
    return LGN2_GPU_LAGUNA_DENSE_Q8_ROUTE_BATCH_FUSED;
}
 ...
 * =========================================================================
 *
 * Opaque device tensor used by the LGN2-specific GPU executor.
 *
 ...
 */
#ifndef LGN2_GPU_TENSOR_DEFINED
#define LGN2_GPU_TENSOR_DEFINED
typedef struct lgn2_gpu_tensor lgn2_gpu_tensor;
#endif

#ifndef LGN2_GPU_ATTENTION_DECODE_ROW_DEFINED
#define LGN2_GPU_ATTENTION_DECODE_ROW_DEFINED
#define LGN2_GPU_ATTENTION_DECODE_BATCH_MAX 32u
typedef struct {
    uint64_t raw_kv;
    uint64_t comp_kv;
    uint64_t topk;
...
#else
static inline int lgn2_gpu_device_is_pre_m5_apple_silicon(void) { return 0; }
static inline int lgn2_gpu_device_is_m5_apple_silicon(void) { return 0; }
#endif
/* =========================================================================
 ...
 * lgn2_gpu_add_rms_norm_weight_rows_tensor API above, while this entry point
 * supplies the three-input add3 variant used by MoE layers. */
static inline int lgn2_gpu_laguna_decode_residual_norm_env_mode(
        const char *value) {
    if (!value || value[0] == '\0' ||
        (value[0] == '0' && value[1] == '\0')) return 0;
    if (value[0] == '1' && value[1] == '\0') return 1;
...
typedef struct lgn2_gpu_laguna_q8_lmhead_screen
 ...
 * LGN2_METAL_LAGUNA_Q8_LMHEAD_SCREEN_V2=1; malformed values are rejected.
 * LGN2_METAL_LAGUNA_Q8_LMHEAD_SCREEN_V2_FALLBACK=1 explicitly permits a v1
 ...
 * collection is captured at plan creation using lgn2_gpu_env_bool (the
 * LGN2_METAL_LAGUNA_Q8_LMHEAD_TRACE or focused-test flag); this function
 ...
 * attention output projections, and LGN2's tail-only RoPE.
 */
...
int lgn2_gpu_test_laguna_decode_route_counters(uint64_t *ordinary,
                                              uint64_t *gqa3,
                                              uint64_t *gqa9,
                                              uint64_t *staged,
 ...
 * batch is active, then complete after lgn2_gpu_end_commands(); production
 ...
 * lgn2_gpu_matmul_f32_decode_rows_exact_tensor followed by
 * lgn2_gpu_glm_router_select_batch_tensor on the supported shape class;
