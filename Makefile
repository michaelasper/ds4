...
test-metal-laguna: check-metal-sources test-lgn test-glm-q23-metal test-laguna-cli-options test-engine-lifecycle ds4 ds4-server ds4-bench ds4-eval
	@set -eu; \
	./ds4_test --laguna-architecture --laguna-selector-parser --laguna-graph-guards --laguna-graph-lifecycle --laguna-dflash-graph-lifecycle --laguna-dflash-command-ownership --server; \
