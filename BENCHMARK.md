...
```zsh
git clone https://github.com/michaelasper/ds4.git ds4-m5-finalists
cd ds4-m5-finalists
git fetch origin laguna-s2.1
git switch --detach origin/laguna-s2.1
git status --short
```

`git status --short` must print nothing. The driver refuses a dirty runbook
checkout.
...
```zsh
export DS4_RELEASE_TAG=bench-laguna-s2.1-m5-max-YYYY-MM-DD-finalists-r1
